...
function buildContextHook(profile: CompanyProfile): string {
  ...
  if (sigs.some(s => (s as any).type === "export")) {
  ...
  if (sigs.some(s => (s as any).type === "ecommerce")) {
  ...
  if (sigs.some(s => (s as any).type === "industry")) {
...
export function buildExecutiveBriefing(
  ...
  return {
    ...
    sinais_chave:    [
      ...profile.operational_signals.slice(0,3).map(s => `${(s as any).label}: ${s.evidence}`),
