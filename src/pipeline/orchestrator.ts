...
import type { ManualEnrichmentInput } from "../enrichment/types"
...
export async function runPipeline(input: PipelineInput): Promise<PipelineResult> {
  ...
  const intelligence: CompanyIntelligence = {
    ...
    public_signals: buildPublicSignals(brasilData, { found: website.data.found, has_ecommerce: website.data.has_ecommerce, has_export: website.data.has_export }, news.data as { items: Array<{ title:string; sentiment:string; tags:string[] }> }, input.segment),
    legal_signals:  [],
...
function buildPublicSignals(brasil: Partial<BrasilAPIData>, website: { found:boolean; has_ecommerce:boolean; has_export:boolean }, news: { items: Array<{title:string; sentiment:string; tags:string[]}>}, segment: string) {
  const signals = []
  ...
  if (website.has_ecommerce && segment !== "servicos") signals.push({ signal: "E-commerce identificado no site", type: "signal" as const, source: "Site", confidence: "medium" as const, commercial_hook: segment === "comercio" ? "E-commerce → DIFAL relevante." : "E-commerce identificado." })
  if (website.has_export && segment !== "servicos") signals.push({ signal: "Operação exportadora sinalizada", type: "signal" as const, source: "Site", confidence: "medium" as const, commercial_hook: "Exportação → IPI Crédito Presumido (5,37%)." })
