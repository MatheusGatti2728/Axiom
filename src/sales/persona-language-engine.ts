...
export function buildCuriosityGap(
  ...
  const topSignal = sigs.find(s => ["export","ecommerce","industry","expansion"].includes((s as any).type)) ?? sigs[0]

  const gap_base = topSignal
    ? `relacionados ao ${(topSignal as any).label?.toLowerCase() ?? "perfil"} de ${nome}`
