...
export function buildUnifiedCopilot(
  ...
  const hypotheses: string[] = profile.operational_signals
    .filter(s => s.confidence !== "high" || !(s as any).type.includes("industry"))
    .slice(0, 3)
    .map(s => `HIPÓTESE — ${(s as any).label}: confirmar na ligação`)
  ...
  return {
    ...
    call_flow: {
      ...
      with_company_ref: legalMaturity !== "none" && hasLitigation
        ? `Identifiquei que ${nome} já possui uma estrutura jurídico-tributária${hasLitigation ? ` com histórico de discussões em temas tributários relevantes` : ""}. O ponto que quero contextualizar é complementar ao que já existe — não uma revisão básica.`
        : opening.with_company_ref,
    ...
    legal_context: {
      ...
      avoid_in_opening:  (legalIntel?.commercial_impact?.avoid ?? []),
